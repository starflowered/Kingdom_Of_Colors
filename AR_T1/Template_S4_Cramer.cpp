...
//
// 	namedWindow(kWinName4, WINDOW_NORMAL);
//
...
// 				// i -> Edge points
// 				/* TASK: Fill in the correct parameters for fitline -> HINT: Comments above */
// 				fitLine(/*???*/);
...
// 				/* TASK: How do we access the correct values -> HINT: How did we do it above when we previously created the lines after fitline*/
...
//
//
// 				/* TASK: We now do the Cramer's rule approach. Have a look at it and do the tasks inside of the block. */
// 				/* TASK: Because Cramer's rule is quite complicated, you could go for a simple strategy by intersecting lines as you did back in school -> Therefore, why not implement such a solution? */
//
//
//
//
...
//
// 				// TASK: c -> What could go wrong? What do we need to check for? -> HINT: Look at what we do next with c
...
//
// 				/* TASK: Did you forget how to create a circle? Create for each new corner a yellow point */
// 				/* ??? */;
//
...
// 			homographyMatrix = getPerspectiveTransform(/* ??? */);
//
...
//
// 			/* TASK: Have a look at the "magical" loop here, you can implement a simpler solution by going around the marker one by one. How would it work? */
