...
string stripe_window      = "Stripe Window";
string contours_window    = "Contours Window";
string marker_window      = "Marker Window";
string threshold_window   = "Threshold Window";
string threshold_label    = "Threshold";
string bw_threshold_label = "BW Threshold";
...
void compute_stripe_intensities(const Mat& img_filtered, const stripe& stripe, Mat image_pixel_stripe, const Point2f p, const int height);

void apply_sobel_y(const Mat& image_pixel_stripe, Mat& grad_y, int& max_intensity_index);

void compute_stripe_edge_center(Mat frame, const stripe& stripe, Point2f edge_point_centers[6], const int j, const Point2f p, Mat grad_y, int& max_intensity_index);

void fit_line_to_edge(Mat frame, float line_params[16], Mat line_params_matrix, const int i, Point2f edge_point_centers[6]);

void compute_corners(Mat frame, float line_params[16], Point2f(& corners)[4]);

void map_marker_to_6x6_image(Mat img_filtered, Point2f corners[4], Mat& image_marker);

bool get_marker_bit_matrix(Mat image_marker, Mat& code_pixel_mat);

bool compute_pnp(Mat frame, aruco::Dictionary aruco_dict, vector<marker> marker_list, Point2f corners[4], Mat code_pixel_mat, Mat_<float>& t_vec);
